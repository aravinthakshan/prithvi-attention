...
class InferenceManager:
    """Manages memory-efficient model inference with KV caching support.
    
    KV caching for faster repeated predictions.
    """
    ...
    def __init__(self, enc_name: str, out_dim: int, out_no_seq: bool = False):
        ...
        # KV cache storage
        self.kv_cache = {}  # Cache for training data KV pairs
    ...
    def configure(
        ...
        """Configure inference parameters with KV caching support."""
        ...
        self.cache_enabled = enable_kv_cache or cache_trainset_representation
    ...
    def _get_cache_key(self, inputs: OrderedDict, train_size: Optional[int] = None) -> str:
        """
        FEATURE ADDITION: Generate cache key from inputs.
        
        Creates a hashable key based on input tensor shapes and train_size.
        """
        key_parts = []
        for name, value in inputs.items():
            if isinstance(value, torch.Tensor):
                key_parts.append(f"{name}:{tuple(value.shape)}:{value.device}")
            else:
                key_parts.append(f"{name}:{value}")
        if train_size is not None:
            key_parts.append(f"train_size:{train_size}")
        return "|".join(key_parts)
    
    def clear_cache(self):
        """FEATURE ADDITION: Clear KV cache."""
        self.kv_cache.clear()
    ...
    def __call__(
        ...
        cached_kv: Optional[Dict] = None,
    ) -> Tensor:
        """
        Forward pass with KV caching support.
        
        If cache_enabled and cached_kv provided, uses cached KV pairs.
        """
        # Check cache if enabled
        if self.cache_enabled and cached_kv is not None:
            # Use cached KV pairs - modify forward_fn to accept cached_kv
            # This requires forward_fn to support cached_kv parameter
            if hasattr(forward_fn, '__self__'):
                # Method call
                if hasattr(forward_fn.__self__, 'forward_with_cache'):
                    return forward_fn.__self__.forward_with_cache(**inputs, cached_kv=cached_kv)
        ...
        *batch_dims, seq_len, in_dim = first_value.shape
        ...
        # 🔧 NEW: If we will batch, probe the true output shape (handles seq_len changes)
        if (not self.out_no_seq) and (batch_size < total_bs):
            probe_slice = tuple(slice(0, 1) for _ in batch_dims)
        
            probe_inputs = {}
            for name, value in inputs.items():
                if isinstance(value, torch.Tensor):
                    probe_inputs[name] = self.to_exe_device(value[probe_slice])
                else:
                    probe_inputs[name] = value
        
            with torch.no_grad():
                if self.use_amp and self.exe_device.type == "cuda":
                    with torch.autocast(device_type="cuda"):
                        probe_out = forward_fn(**probe_inputs)
                else:
                    probe_out = forward_fn(**probe_inputs)
        
            tail = probe_out.shape[len(batch_dims):]   # e.g. (T_test, out_dim)
            output_shape = (*batch_dims, *tail)
            del probe_out, probe_inputs
