"""Define argument parser for OrionBix training."""
...
def build_parser():
    """Build parser with all OrionBix training arguments."""
    ...
    parser.add_argument("--wandb_project", type=str, default="OrionBix", help="Wandb project name")
    ...
    ###########################################################################
    parser.add_argument("--meta_learning", default=False, type=str2bool, help="Enable meta-learning with k-NN support selection")
    parser.add_argument("--support_size", type=int, default=32, help="Size of support set in each episode")
    parser.add_argument("--query_size", type=int, default=640, help="Total size of query set (will be split across episodes)")
    parser.add_argument("--k_neighbors", type=int, default=64, help="Number of k-NN neighbors to select for each anchor")
    parser.add_argument("--similarity_metric", type=str, default="cosine", choices=["cosine", "euclidean", "manhattan"], help="Similarity metric for k-NN selection")
    parser.add_argument("--feature_normalization", default=True, type=str2bool, help="Normalize features before computing similarity")
    parser.add_argument("--meta_accumulation_steps", type=int, default=4, help="Number of gradient accumulation steps for meta-learning")
    parser.add_argument("--num_episodes_per_dataset", type=int, default=20, help="Number of episodes to create per dataset (also serves as B for LocalPFN-style k-NN selection)")

    parser.add_argument("--diversity_factor", type=float, default=0.3, help="Factor to encourage diverse k-NN selection")
    parser.add_argument("--min_dataset_size", type=int, default=128, help="Minimum dataset size to generate episodes")
    parser.add_argument("--episode_method", type=str, default="random", choices=["random", "knn"], help="Episode generation method: 'random' for simple splits, 'knn' for k-NN with diversity")
    
    ###########################################################################
    ###### Linear Attention ######################################################
    ###########################################################################
    
    parser.add_argument("--col_attention_type", type=str, default="standard", choices=["linear", "standard"], 
                       help="Type of attention for column stage: linear or standard")
    parser.add_argument("--col_feature_map", type=str, default="identity", choices=["elu", "identity", "hedgehog"],
                       help="Feature map for linear attention in column stage")
    parser.add_argument("--row_attention_type", type=str, default="standard", choices=["linear", "standard","bi_axial"],
                       help="Type of attention for row stage: linear or standard")
    parser.add_argument("--row_feature_map", type=str, default="identity", choices=["elu", "identity", "hedgehog"],
                       help="Feature map for linear attention in row stage")
    parser.add_argument("--icl_attention_type", type=str, default="standard", choices=["linear", "standard"],
                       help="Type of attention for ICL stage: linear or standard")
    parser.add_argument("--icl_feature_map", type=str, default="identity", choices=["elu", "identity", "hedgehog"],
                       help="Feature map for linear attention in ICL stage")
