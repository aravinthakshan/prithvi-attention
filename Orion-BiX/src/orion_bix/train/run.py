...
class Trainer:
    """This class handles the complete training lifecycle for OrionBix
    ...
    def __init__(self, config):
        ...
        # --- NEW: for ETA smoothing (exponential moving average of step time) ---
        self._ema_step_sec = None
        self._ema_alpha = 0.10  # 0.10 = react in ~10 steps
    ...
    def train(self):
        ...
        for step in step_progress:
            ...
            ## NEW For ETA
            total_step_sec = prior_time + train_time
            # Update EMA of total step time for a stable ETA
            ...
            ## NEW
            steps_left = self.config.max_steps - self.curr_step
            eta_sec = max(0.0, self._ema_step_sec * steps_left)  # seconds
            # Throughput helpers (avoid div-by-zero)
            ...
            if self.master_process:
                ...
                results.update({
                    ...
                    "steps_per_sec": total_sps,            # end-to-end (prior+train)
                    "train_steps_per_sec": train_sps,      # train-only
                    "eta_sec": eta_sec,                    # estimated remaining time (seconds)
                    "eta_min": eta_sec / 60.0,             # same, minutes
                })
...
if __name__ == "__main__":
    ...
    # Choose trainer based on meta-learning configuration
    if config.meta_learning:
        from orion_bix.train.meta_trainer import MetaLearningTrainer
